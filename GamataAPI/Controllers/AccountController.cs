...
namespace GamataAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> Insert([FromBody] InsertAccount model)
        {
            try
            {
                return Ok(await _userService.RegisterAsync(new InsertAccount()
                {
                    ...
                    Role = model.Role,
                    ModifiedBy = User.Identities.First().Claims.FirstOrDefault(s => s.Type == "uid")?.Value ?? "",
                    ModifiedDateTime = DateTime.Now,
                    Address = model.Address,
                    Designation = model.Designation,
                    Join_date = (DateTime)model.Join_date,
                    NIC_no = model.NIC_no,
        ...
        public async Task<IActionResult> UserSignUp([FromBody] InsertAccount model)
        {
            try
            {
                return Ok(await _userService.UserSignUpAsync(new InsertAccount()
                {
                    ...
                    Address = model.Address,
                    Designation = model.Designation,
                    Join_date = (DateTime)model.Join_date == Convert.ToDateTime("1/1/0001 12:00:00 AM") ? CommonResources.LocalDatetime() : (DateTime)model.Join_date,
                    NIC_no = model.NIC_no,
        ...
        public async Task<IActionResult> UpdateLockAccount([FromBody] EditAccount model)
        {
            try
            {
                model.ModifiedBy = User.Identities.First().Claims.FirstOrDefault(s => s.Type == "uid")?.Value ?? "";
                model.ModifiedDateTime = DateTime.Now;
